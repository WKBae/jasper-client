...
class GoogleCloudSTT(AbstractSTTEngine):
    """
    Speech-To-Text implementation which relies on the Google Cloud Speech API.
    ...
    Excerpt from sample profile.yml:
        ...
        ...
        timezone: US/Pacific
        stt_engine: google-cloud
        keys:
            GOOGLE_CLOUD_SPEECH: $YOUR_KEY_HERE

    """

    SLUG = 'google-cloud'

    def __init__(self, api_key=None, language='ko-KR'):
        # FIXME: get init args from config
        """
        Arguments:
        api_key - the public api key which allows access to Google APIs
    ...
    def _regenerate_request_url(self):
        if self.api_key and self.language:
            query = urllib.urlencode({'key': self.api_key})
            self._request_url = urlparse.urlunparse(
    ...
    def get_config(cls):
        ...
        if os.path.exists(profile_path):
            with open(profile_path, 'r') as f:
                profile = yaml.safe_load(f)
                if 'keys' in profile and 'GOOGLE_CLOUD_SPEECH' in profile['keys']:
                    config['api_key'] = profile['keys']['GOOGLE_CLOUD_SPEECH']
        return config
    ...
    def transcribe(self, fp):
        ...
        r = self._http.post(self.request_url, data=content, headers=headers)
        try:
            r.raise_for_status()
        except requests.exceptions.HTTPError:
            self._logger.critical('Request failed with http status %d',
                                  r.status_code)
            self._logger.critical(r.text)
            if r.status_code == requests.codes['forbidden']:
                self._logger.warning('Status 403 is probably caused by an ' +
                                     'invalid Google API key.')
            return []
